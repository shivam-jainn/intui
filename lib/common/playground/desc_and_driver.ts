...
export async function getDesc(question_slug: string) {
  ...
  try {
    ...
    if (error.code === 'ENOENT' || error.status === 404) {
      ...
      throw new Error(
        'Unable to connect to storage service. Please check your internet connection.'
      );
    } else {
      throw new Error(
        `Failed to load question description for "${question_slug}". Please try again later.`
      );
    }
  }
}
